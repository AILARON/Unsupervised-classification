###################
#Bag of feature layer provided by Passalis et al. https://github.com/passalis/keras_cbof
#Fixed to work in tensorflow 2.0
###################
...
class BoF_Pooling(Layer):
    ...
    def build(self, input_shape):
        self.V = self.add_weight(name='codebook', shape=(1, 1, input_shape[3], self.N_k), initializer='uniform',
                                 trainable=True)
        self.sigmas = self.add_weight(name='sigmas', shape=(1, 1, 1, self.N_k),initializer = tf.constant_initializer(0.1),
                                      trainable=True)
...
def initialize_bof_layers(model, data, n_samples=100, n_feature_samples=5000, batch_size=32, k_means_max_iters=300,
    ...
    for i in range(len(model.layers)):
        if isinstance(model.layers[i], BoF_Pooling):
            ...
            get_features = K.function([model.input], [model.layers[i - 1].output])
            #get_features = Model([model.input], [model.layers[i - 1].output])
